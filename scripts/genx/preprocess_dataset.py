...
def labels_and_ev_repr_timestamps(npy_file: Path,
    ...
    for idx, (num_ev_repr_between, frame_ts_us_start, frame_ts_us_end) in enumerate(zip(
        ...
        # 就是在一个frame 的时间戳范围内 分成num_ev_repr_between等分 new_edge_timestamps就是事件的时间戳起点/终点的列表
    ...
    # 输出的 ev_repr_timestamps_us 就是 ev_repr_timestamps_us_end
...
def write_event_representations(in_h5_file: Path,
    ...
    with H5Reader(in_h5_file, dataset=dataset) as h5_reader, \
        ...
        # ev_repr_timestamps_us是每个事件帧结尾的时间戳, 返回的是要插入元素的索引
        end_indices = np.searchsorted(ev_ts_us, ev_repr_timestamps_us, side='right')
        if ev_repr_num_events is not None:
            start_indices = np.maximum(end_indices - ev_repr_num_events, 0) # 计算结果和0取最大值，确保起始位置索引不会小于0
        else:
    ...
    try:
        labels_per_frame, frame_timestamps_us, ev_repr_timestamps_us, frameidx2repridx = \
            labels_and_ev_repr_timestamps(
                ...
                ts_step_ev_repr_ms=ts_step_ev_repr_ms,  # 每个事件帧的结尾时间戳
...
if __name__ == '__main__':
    ...
    parser.add_argument('-input_dir', default='/data2/zht/normal_detection/EVENT/GEN1')
    parser.add_argument('-target_dir', default='/home/wxy_g/workspace/RVT/tmp')
    parser.add_argument('-ev_repr_yaml_config', default= 'conf_preprocess/representation/stacked_hist.yaml', 
                        help='Path to event representation yaml config file')
    parser.add_argument('-extraction_yaml_config', default= 'conf_preprocess/extraction/const_duration.yaml',
                        help='Path to event window extraction yaml config file')
    parser.add_argument('-bbox_filter_yaml_config', default= 'conf_preprocess/filter_gen4.yaml', 
    ...
    print(colored('ev_repr_string:{}'.format(ev_repr_string), 'yellow'))
    ...
    for split in [train_path, val_path, test_path]:
        ...
        for npy_file in split.iterdir():
            ...
            sequence_data = {
                DataKeys.InNPY: npy_file,  # label
                DataKeys.InH5: h5f_path,  # raw event
