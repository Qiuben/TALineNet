...
if torch_is_version_1:
    ...
    def collate_tensor_fn(batch, *, collate_fn_map: Optional[Dict[Union[Type, Tuple[Type, ...]], Callable]] = None):
        ...
        #  添加调试信息
        # print(colored(f"Element type: {type(elem)}", 'green'))
        # print(colored(f"Batch types: {[type(x) for x in batch]}", 'blue'))
        # print(colored(f"Output tensor type: {type(out)}", 'yellow'))
        # print(colored(f"Element dtype: {elem.dtype}", 'cyan'))
        # print(colored(f"Batch dtypes: {[x.dtype for x in batch]}", 'magenta'))
