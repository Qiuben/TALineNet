...
class ObjectLabelBase:
    ...
    @property
    def x1(self):
        return self.object_labels[:, self._str2idx['x1']]
    
    @property
    def y1(self):
        return self.object_labels[:, self._str2idx['y1']]
    
    @property
    def x2(self):
        return self.object_labels[:, self._str2idx['x2']]

    @property
    def y2(self):
        return self.object_labels[:, self._str2idx['y2']]
    

    @x1.setter
    def x1(self, value: Union[th.Tensor, np.ndarray]):
        self.object_labels[:, self._str2idx['x1']] = value


    @y1.setter
    def y1(self, value: Union[th.Tensor, np.ndarray]):
        self.object_labels[:, self._str2idx['y1']] = value


    @x2.setter
    def x2(self, value: Union[th.Tensor, np.ndarray]):
        self.object_labels[:, self._str2idx['x2']] = value

    @y2.setter
    def y2(self, value: Union[th.Tensor, np.ndarray]):
        self.object_labels[:, self._str2idx['y2']] = value
...
class ObjectLabelFactory(ObjectLabelBase):
    ...
    def from_structured_array(object_labels: np.ndarray,
        ...
        t_array= object_labels['t'].astype('float32')
        lines_array = object_labels['lines'].astype('float32')

        sx = 128 / original_size[1]
        sy = 128 / original_size[0]

        lines_array[:, 0] = lines_array[:, 0] * sx
        lines_array[:, 1] = lines_array[:, 1] * sy
        lines_array[:, 2] = lines_array[:, 2] * sx
        lines_array[:, 3] = lines_array[:, 3] * sy

        # 将两个数组按列合并
        np_labels = np.concatenate((t_array.reshape(-1, 1), lines_array), axis=1)
    ...
    def __getitem__(self, item: int) -> ObjectLabels:
        ...
        # 这里 self.object_labels 的形状应该是 [N, 5]
...
class ObjectLabels(ObjectLabelBase):
    def __init__(self,
        ...
        super().__init__(object_labels = object_labels, input_size_hw = input_size_hw)
        self.seq_name: Optional[str] = seq_name

        self.item: Optional[int] = item
    def __len__(self) -> int:
        return self.object_labels.shape[0]

    def rotate_clockwise_(self):
        width = self.input_size_hw[1]   
        height = self.input_size_hw[0]

        ori_x1 = copy.deepcopy(self.x1)
        ori_x2 = copy.deepcopy(self.x2)

        ori_y1 = copy.deepcopy(self.y1)
        ori_y2 = copy.deepcopy(self.y2)
        
        self.x1 = (height - 1 - ori_y1) * (width/height) 

        self.y1 = ori_x1 * (height/width)

        self.x2 = (height - 1 - ori_y2) * (width/height) 

        self.y2 = ori_x2 * (height/width)

       
    def rotate_counterclockwise_(self):
        ...
        self.x1 = ori_y1 * (width/height)
        self.x2 = ori_y2 * (width/height)

        self.y1 = (width-ori_x1) * (height/width)
        self.y2 = (width-ori_x2) * (height/width)
    ...
    def flip_h_(self):  # 水平翻转
        if len(self) == 0:
            return
        
        self.x1 = self.input_size_hw[0] - self.x1
        self.x2 = self.input_size_hw[0] - self.x2
        
    def flip_v_(self): #垂直翻转
        self.y1 = self.input_size_hw[1] - self.y1
        self.y2 = self.input_size_hw[1] - self.y2
...
class SparselyBatchedObjectLabels:
    ...
    def flip_h_(self):
        for idx, entry in enumerate(self.sparse_object_labels_batch):
            if entry is not None:
                self.sparse_object_labels_batch[idx].flip_h_()

    def flip_v_(self):
        for idx, entry in enumerate(self.sparse_object_labels_batch):
            if entry is not None:
                self.sparse_object_labels_batch[idx].flip_v_()

    def rotate_clockwise_(self):
        for idx, entry in enumerate(self.sparse_object_labels_batch):
            if entry is not None:
                self.sparse_object_labels_batch[idx].rotate_clockwise_()

    def rotate_counterclockwise_(self):
        for idx, entry in enumerate(self.sparse_object_labels_batch):
            if entry is not None:
                self.sparse_object_labels_batch[idx].rotate_counterclockwise_()
    ...
    def get_valid_labels_and_batch_indices(self) -> Tuple[List[ObjectLabels], List[int]]:
        ...
        for idx, label in enumerate(self.sparse_object_labels_batch):
            if label is None:
                label = self.sparse_object_labels_batch[0]
