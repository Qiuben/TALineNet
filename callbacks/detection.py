...
from utils.evaluation.prophesee.visualize.vis_utils import draw_lines_with_labels
...
class DetectionVizCallback(VizCallbackBase):
    ...
    def on_train_batch_end_custom(self,
        ...
        #     #绘制预测结果
        #     predictions = outputs[ObjDetOutput.PRED_PROPH][sample_idx]
        #     prediction_img = ev_img.copy()
        #     # draw_bboxes(prediction_img, predictions_proph, labelmap=self.label_map)
        #     draw_lines(prediction_img, predictions)

        #     # 绘制gt
        #     labels = outputs[ObjDetOutput.LABELS_PROPH][sample_idx]
    ...
    def on_validation_batch_end_custom(self, batch: Any, outputs: Any):
        ...
        # 绘制predictions
        predictions = outputs[ObjDetOutput.PRED_PROPH]  # [N, 2, 2]
        prediction_img = ev_img.copy()
        save_name = 'predict'
        prediction_img = draw_lines_with_labels(prediction_img, predictions, save_name)
        self.add_to_buffer(DetectionVizEnum.PRED_IMG_PROPH, prediction_img)

        # 绘制gt
        gt = outputs[ObjDetOutput.LABELS_PROPH]
        gt_line = gt[:, 1:5].reshape(-1, 2, 2)
        ...
        save_name = 'gt'
        gt_img = ev_img.copy()
        gt_img = draw_lines_with_labels(gt_img, gt_line, save_name)
  
        self.add_to_buffer(DetectionVizEnum.LABEL_IMG_PROPH, gt_img)
