        ...
        with torch.no_grad():
            ...
            for i in range(b):
                ...
                if self.training:
                    ...
                    if junc_pred.shape[0] == 0:
                        device = jmap.device
                        # print(f'WARNING: Invalid shape {junc_pred.shape}')
                        junc_pred = torch.full((1, 2), 1e6, device=device) 
    ...
    def sample_lines(self, loi_preds, loi_scores, metas):
        ...
        * math:`O`: order of the Bezier curve.
        ...
        for i in range(b):
            ...
            lpre = metas['lpre'][i]   # (N, 5, 2)
            lpre_label = metas['lpre_label'][i]
            line = metas['line'][i]   # (N, 5, 2)
