...
def calc_APJ_optimized(junc_gts, junc_preds, im_ids, distances):
    # GT 点的总数量
    n_gt = sum(junc_gt.shape[0] for junc_gt in junc_gts)
    n_pred = junc_preds.shape[0]

    # KD-trees for GT junctions
    kdtrees = [cKDTree(junc_gt) for junc_gt in junc_gts]

    # 初始化结果
    results = []

    for distance in distances:
        # 初始化 True Positive 和 False Positive
        tp, fp = np.zeros(n_pred, dtype=float), np.zeros(n_pred, dtype=float)
        # 标记 GT 是否被匹配
        hits = [np.zeros(junc_gt.shape[0], dtype=bool) for junc_gt in junc_gts]
        ...
        for i in range(n_pred):
            ...
            # 查询 KD-tree 获取最近的 GT 点
            dists, indices = kdtrees[im_id].query(junc_pred, k=1)

            # 判断是否是匹配
            if dists < distance and not hits[im_id][indices]:
                tp[i] = 1
                hits[im_id][indices] = True
            else:
                fp[i] = 1
