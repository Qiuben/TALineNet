function eval_release(image_path, temp_path, output_file, output_size)
...
listing = dir([temp_path, '/gt/*.mat']);
numResults = size(listing, 1);
for index=1:numResults
  filename = listing(index).name;
  filename = filename(1:end-4);
  
  %if strcmp(string(filename), '00113155')
  %	fprintf('1')
  %   continue;
  %end
  fprintf('processed %d/%d\n', index, numResults)
  gtname = [temp_path, '/gt/', filename, '.mat'];
  ...
  for k = 1:ne
    x1 = gtlines(k,1) * width / output_size;
    x2 = gtlines(k,3) * width / output_size;
    y1 = gtlines(k,2) * height / output_size;
    y2 = gtlines(k,4) * height / output_size;
  ...
  parfor m=1:nLineThresh
    resultname = [temp_path, '/', num2str(lineThresh(m)), '/', filename, '.mat'];
